...
export default function DeleteMasterButton({
  ...
  const t = useTranslations('admin.deleteMaster')
  ...
  function handleDelete() {
    startTransition(async () => {
      ...
      if (res.ok) {
        toast.success(t('deleted', { name: res.name ?? masterName }))
        setConfirming(false)
      } else {
        const code = CODE_KEYS.has(res.code) ? res.code : 'unexpected_error'
        toast.error(t(`codeMessages.${code}`))
      }
    })
  }
    ...
    return (
      <div className="w-60 rounded-xl border bg-white p-3 text-left shadow-lg">
        <p className="text-xs leading-relaxed text-gray-700">
          {t('confirmPrefix')}
          <span className="font-semibold">{masterName}</span>{t('confirmSuffix')}
        </p>
  ...
  return (
    <button
      ...
      aria-label={t('deleteAria', { name: masterName })}
      title={t('deleteTitle')}
