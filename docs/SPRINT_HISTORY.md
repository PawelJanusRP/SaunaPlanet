...
| SP-035 | In Review   | `feature/sp-035-master-studio`    | —         | Master Studio Foundation — /studio on the shared shell, profile integrity (unique account↔profile link, own-row RLS, privileged-column guard), first-class master_affiliations with two-direction lifecycle, Owner Workspace Team module; home_sauna_id frozen as legacy (SQL to apply manually) |
