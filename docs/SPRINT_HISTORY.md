...
| SP-047 | Completed — deployed 2026-09-09 | `feature/sp-047-i18n-pl-en-de` (merged, `3f27922`) | `3f27922` | Internationalization & Localization (PL/EN/DE) on next-intl — locale-prefixed canonical URLs, root locale negotiation, permanent legacy→/pl redirects, file-based catalogs, shared language selector (Navbar + map menu), localized UI + server-action/pure-lib messages, locale-aware date/number/PLN formatting, international SEO (per-locale canonical + hreflang + x-default root + multilingual sitemap + robots), global Help hub. Zero production DB migrations; UGC untranslated; SP-044 privacy + SP-045 map preserved. Slices A–D + E1 (security-sensitive lib messages) + E2 (final UI labels + Help nav) + E3 (formatting + SEO). **CLOSED / DEPLOYED / VERIFIED on production (sauna-planet.pl) 2026-09-09.** Architecture: docs/SP047_I18N_ARCHITECTURE.md |
