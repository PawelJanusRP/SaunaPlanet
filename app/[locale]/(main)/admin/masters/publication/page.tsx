...
function toFilter(raw: string | undefined): string {
  return FILTER_IDS.some((id) => id === raw) ? (raw as string) : 'submitted'
}
    ...
    <main className="mx-auto max-w-5xl p-4">
      ...
      <div className="mb-6 flex flex-wrap gap-1.5">
        {FILTER_IDS.map((id) => (
          <Link
            key={id}
            href={`/admin/masters/publication?filter=${id}`}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 flex flex-wrap gap-x-3 text-xs text-gray-500">
                    ...
                    <span>{t('profile', { status: m.status })}</span>
                    <span>{m.user_id !== null ? t('hasOwner') : t('noOwner')}</span>
                    <span>
                      {missing.length === 0
                        ? t('complete')
